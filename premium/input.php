<?php
		...
		if($pay=='Yes'){
			...
			mysqli_query($dbc,$query2) or die($query2."Can't Connect to Query...");
			$query3="update unpaid_premium set Fine=0,Lateness=0 where Policy_Num='$pn' ";
			mysqli_query($dbc,$query3) or die($query3."Can't Connect to Query...");
		}
		else if($pay=='No'){
			$now = time();
			...
			mysqli_query($dbc,$query1) or die($query1."Can't Connect to Query...");
		}
